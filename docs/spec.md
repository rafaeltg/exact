...
After the `llm` total line and its cache line, and before the `exa` line, the footer prints two groups: `by model`, then `by node`. Each group has one row for each raw `model` or `node` value on the `llm` events. A missing or empty value groups as `(unknown)`. A row copies the total row format: calls, input tokens, output tokens and USD. A row prints its own cache sub-line only when its `cache_read` or `cache_creation` is not zero. The sub-line has the format of the total cache line. A `by model` row shows the word `unpriced` in place of the USD when the rate table cannot price that model. A `by node` row adds the USD of its priced events. It appends ` + K unpriced` when some of its events are unpriced, as the total line does. Rows sort by priced USD, highest first. Equal USD sorts by input plus output tokens, highest first. Equal tokens sort by key, in ascending order. A row with no priced USD sorts as USD `0.0`. In each group, the unrounded priced USD of the rows adds up to the unrounded `llm` USD. The printed `.4f` values can differ from the printed total by rounding. Neither group prints when the run has no `llm` events. Only the `## Usage` footer prints the groups. The JSONL trace sidecar, `run_end` and the `--verbose` status lines do not change.
