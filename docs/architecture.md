...
`usage` events are LLM and vendor call counts/tokens. The CLI prints a `## Usage` footer with estimated USD (dated rates in `usage.py`), including cache_read / cache_creation when present (Anthropic cache multipliers). Elicit calls are counted at $0. Vendor retries are not metered. After the `llm` total line and its cache line, the footer prints two groups: `by model`, then `by node`. Each group has one row for each raw `model` or `node` value on the `llm` events. A missing or empty value groups as `(unknown)`. Rows sort by priced USD (highest first), then by tokens (highest first), then by key. A `by model` row shows `unpriced` when the rate table has no rate for its model. A `by node` row adds the USD of its priced events and appends ` + K unpriced` for its unpriced events. In each group, the unrounded USD of the rows adds up to the `llm` total. Neither group prints when the run has no `llm` events. Only this footer prints the groups. See spec §7b.
