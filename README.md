...
cp .env.example .env   # set EXA_API_KEY and ANTHROPIC_API_KEY
...
```
Default model: `anthropic:claude-haiku-4-5`. Optional per-role overrides: `EXACT_MODEL_*`, `EXACT_MAX_TOKENS_*`, plus `EXACT_TEMPERATURE`, `EXACT_THINKING_BUDGET` (see `.env.example` / architecture).
